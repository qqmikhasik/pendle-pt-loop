...
class MorphoMarketLoader(Loader):
    ...
    def transform(self) -> None:
        """Convert the staging frame to the documented index/column shape."""
        ...
        if self._data is None or self._data.empty:
            ...
            return
        df = self._data.copy()
        df["time"] = pd.to_datetime(df["x"].astype("int64"), unit="s", utc=True)
        df = df.drop(columns=["x"]).set_index("time").sort_index()
        # Ensure every documented column exists even if the API omitted it.
        for c in cols:
    ...
    def read(self, with_run: bool = False) -> pd.DataFrame:
        """Return the hourly DataFrame; run the pipeline on cache miss."""
        if with_run:
            self.run()
        else:
    ...
    def _restore_index(self) -> None:
        """Re-attach a UTC DatetimeIndex to a CSV-read DataFrame."""
        if self._data is None or self._data.empty:
